...
class PanoramaAction extends WapAction{
	...
	public function houselist(){
		...
		$item=$t_housetype_360->where(array('housetype_id'=>intval($_GET['id']),'token'=>$this->token))->select();
		$t_housetype = M('Estate_housetype');
		$check = $t_housetype->where(array('id'=>intval($_GET['id']),'token'=>$this->token))->find();
		$this->assign('info',$check);
		$this->assign('floors',$item);
		//print_r($item);die();
		$this->display();
	}
	public function show(){
		$t_housetype_360 = M('Estate_housetype_360');
		$item=$t_housetype_360->where(array('id'=>intval($_GET['id']),'token'=>$this->token))->find();
		$this->assign('item',$item);
		$this->display();
	}
}
