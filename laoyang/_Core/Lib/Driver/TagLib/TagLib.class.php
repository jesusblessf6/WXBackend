...
class TagLibCx extends TagLib {
    ...
    protected $tags   =  array(
        ...
        'laoyangif'        =>  array('attr'=>'where','level'=>2),
		...
		'laoyanglist'    =>  array('attr'=>'array,foreach,offset,length,key,mod','level'=>3,'alias'=>'iterate'),
	...
	public function _laoyangif($attr,$content) {
        $tag          = $this->parseXmlAttr($attr,'laoyangif');
	...
	public function _laoyanglist($attr,$content) {
        static $_iterateParseCache = array();
        //如果已经解析过，则直接返回变量值
        $cacheIterateId = md5($attr.$content);
        if(isset($_iterateParseCache[$cacheIterateId]))
            return $_iterateParseCache[$cacheIterateId];
        $tag      = $this->parseXmlAttr($attr,'laoyanglist');
        $name   = $tag['array'];
        $id        = $tag['foreach'];
        $empty  = isset($tag['empty'])?$tag['empty']:'';
        $key     =   !empty($tag['key'])?$tag['key']:'i';
