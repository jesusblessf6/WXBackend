	...
	public function catAdd()
	{
		...
		if (IS_POST) {
			...
			if (D('Product_cat')->add($_POST)) {
				...
				$this->success('添加成功', U('Store/index', array('token' => session('token'), 'cid' => $this->_cid, 'parentid' => $parentid)));
			}
		} else {
			$parentid = isset($_GET['parentid']) ? intval($_GET['parentid']) : 0;
			if ($parentid) {
...
//	        }
	...
	public function productSave() {
		...
		$data = array('token' => $token, 'gid' => $gid, 'fakemembercount'=>$fakemembercount,'status' => $status, 'cid' => $this->_cid, 'num' => $num, 'sort' => $sort, 'catid' => $catid, 'name' => $name, 'price' => $price, 'mailprice' => $mailprice, 'vprice' => $vprice, 'oprice' => $oprice, 'intro' => $intro, 'logourl' => $pic, 'keyword' => $keyword, 'time' => time());
		...
		if ($keys = M('Keyword')->where(array('pid' => $pid, 'token' => $token, 'module' => 'Store'))->find()) {
			M('Keyword')->where(array('pid' => $pid, 'token' => $token, 'id' => $keys['id']))->save(array('keyword' => $keyword));
		} else {
			M('Keyword')->add(array('token' => $token, 'pid' => $pid, 'keyword' => $keyword, 'module' => 'Store'));
		}
		if (!empty($attribute)) {
	...
	public function orderInfo()
	{
		...
		if (IS_POST){
			...
			if ($_POST['sent']) {
				$thisWxUser = M('Wxuser')->where(array('token' => $thisOrder['token']))->find();
				if ($thisWxUser['winxintype'] == 3)
				{
				...
				Sms::sendSms($thisOrder['token'], "您在{$company['name']}商城购买的商品，商家已经给您发货了，请您注意查收", $userInfo['tel']);
			}
			
			//货到付款改变支付状态增加积分
			/************************************************/
			if (intval($_POST['paid'])&&intval($thisOrder['price'])&&intval($thisOrder['paymode'])==1){
