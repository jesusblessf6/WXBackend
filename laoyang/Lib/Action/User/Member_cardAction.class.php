 ...
 public function create_add()

    {
        ...
        if (IS_POST) {
			...
			while($j<=$num){
				$row = array();
				$row['cardid'] = $this->thisCard['id'];
				$row['token']  = session('token');
				$row['number']   = $_POST['title'] . ($stat += 1);
	...
	*/
	...
	public function addDetail($rows){
		...
		//数据量较大,采取批量插入
		$data = implode(',', $rows);
		$sql = "INSERT IGNORE INTO wy_member_card_create(cardid,token,number)
				 VALUES {$data}";
	...
	public function coupon_edit(){
		...
		if(IS_POST){
			$data=array();
			$enddates=explode('-',$_POST['enddate']);
			$data['enddate']=mktime(23,59,59,$enddates[1],$enddates[2],$enddates[0]);
			$statdate=explode('-',$_POST['statdate']);
			$data['statdate']=mktime(23,59,59,$statdate[1],$statdate[2],$statdate[0]);			
			...
			$data['token']=$this->thisCard['token'];
			if (!isset($_GET['itemid'])){
				$data['create_time']=time();
				$member_card_coupon_db->add($data);
			}else {
				$member_card_coupon_db->where(array('id'=>intval($_GET['itemid'])))->save($data);
			}
			$this->success('操作成功',U('Member_card/coupon',array('token'=>session('token'),'id'=>$this->thisCard['id'])));
		}else{
			$now=time();
			if (isset($_GET['itemid'])){
	...
	public function integral_edit(){
		...
		if(IS_POST){
			...
			if (!isset($_GET['itemid'])){
				$data['create_time']=time();
				$member_card_inergral_db->add($data);
			}else {
				$member_card_inergral_db->where(array('id'=>intval($_GET['itemid'])))->save($data);
			}
			$this->success('操作成功',U('Member_card/integral',array('token'=>session('token'),'id'=>$this->thisCard['id'])));
		}else{
			$now=time();
			if (isset($_GET['itemid'])){
	...
	public function members(){
		...
		$where['wecha_id']=array('neq','');
		$card_set=M('member_card_set');
		$where1=array();
		$where1['id']=intval($_GET['id']);
		$where1['token']=$this->token;
		$card_set_info=$card_set->where($where1)->find();
		$this->assign('card_set_info',$card_set_info);
		if (IS_POST){
		...
		if ($members){
			...
			foreach ($members as $member){
				...
				if($thisUser['diy1_value'])
				{
					$members[$i]['diyname1']=$thisUser['diy1_value'];
				}
				if($thisUser['diy2_value'])
				{
					$members[$i]['diyname2']=$thisUser['diy2_value'];
				}
				$members[$i]['sex']=$thisUser['sex'];
				$members[$i]['age']=$thisUser['age'];
				$members[$i]['tel']=$thisUser['tel'];
				$members[$i]['getcardtime']=date('Y-m-d',$thisUser['getcardtime']);
            ...
            if(isset($_GET['download'])){
                ...
                $keynames = array('number'=>'会员卡号','truename'=>'真实姓名','wechaname'=>'微信名','qq'=>'QQ','diyname1'=>$card_set_info['diyname1'],'diyname2'=>$card_set_info['diyname2'],'sex'=>'性别','age'=>'年龄','tel'=>'电话','getcardtime'=>'领卡时间','expensetotal'=>'消费金额','total_score'=>'积分');
                $name = "会员数据_".date('Ymd');
                $this -> generate_xls($keynames, $members, $name);
            }
	...
	public function memberExpense(){
		...
		if (IS_POST){
			...
			$arr['score']=$set_exchange['reward']*$arr['expense'];
			//
	...
	public function setdiyname(){
		//score
		if (IS_POST){
			$arr['diyname1']=$this->_post('diyname1');
			$arr['diyname2']=$this->_post('diyname2');
			M('Member_card_set')->where(array('token'=>$this->token,'id'=>$this->thisCard['id']))->save($arr);
			$this->success('操作成功');
		}
		else
		{
			...
			if($name=='born_dis')
			{
				...
				$arr['bornmonth_dis']=$p;
				$arr['bornday_dis']=$p;
				$back=M('Member_card_set')->where(array('token'=>$this->token,'id'=>$this->thisCard['id']))->save($arr);
			}
			else
			{
				$back=M('Member_card_set')->where(array('token'=>$this->token,'id'=>$this->thisCard['id']))->save(array($name=>$p));
			}
			if($back){
				echo 1;
			}else{
...

    {
        ...
        M('Member_card_exchange')->where(array(
