...
class GrouponAction extends UserAction{
	...
	public function add(){ 
		...
		$time=mktime($times[0],$times[1],$times[2],$dates[1],$dates[2],$dates[0]);
		}else {
			$time=0;
		}
		return $time;
	...
	public function set(){
		...
		if(IS_POST){ 
			...
			if($this->product_model->create()){
				$_POST['endtime']=$this->getTime($_POST['enddate']);
				$_POST['starttime']=$this->getTime($_POST['startdate']);
	...
	public function setting(){
		...
		$obj = $setting->where(array('token' => session('token'),'groupon'=>1))->find();
		
		if (IS_POST) {
			if ($obj) {
				$t = $setting->where(array('token' => session('token'),'groupon'=>1))->save($_POST);
				if ($t) {
					$this->success('修改成功',U('Groupon/setting',array('token' => session('token'))));
				} else {
					$this->error('操作失败');
				}
			} else {
				...
				if ($pid) {
					$this->success('增加成功',U('Groupon/setting',array('token' => session('token'))));
				} else {
					$this->error('操作失败');
				}
			}
