...
class StatisticsAction extends UserAction
{
	...
	public function _initialize()
	{
		...
		$this->diymen=M('diymen_set')->where(array('token'=>$_SESSION['token']))->find();
		if (!$this->diymen['appid'] || !$this->diymen['appsecret'])
		{
		...
		$this->modules = array('home' => array('name' => '微网站'), 'text' => array('name' => '文本请求'), 'member_card_set' => array('name' => '会员卡'), 'lottery' => array('name' => '推广活动'), 'help' => array('name' => '帮助'), 'wedding' => array('name' => '婚庆喜帖'), 'img' => array('name' => '图文消息'), 'selfform' => array('name' => '万能表单'), 'host' => array('name' => '通用订单'), 'panorama' => array('name' => '全景'), 'usernamecheck' => array('name' => '账号审核'), 'album' => array('name' => '相册'), 'vote' => array('name' => '投票'), 'product' => array('name' => '商城'), 'voiceresponse' => array('name' => '语音消息'), 'estate' => array('name' => '房产'),);
	}
	public function index()
	{
		switch ($_GET['type'])
		{
			case 1: $group_list_db = M('Wechat_group_list');
				$where = array('token' => $this->token);
				...
				$this->assign('xml', $xml);
				break;
			case 2: $openid = $this->_get('openid');
				$where = array('token' => $this->token);
				if ($openid)
				{
					$where['openid'] = $openid;
				...
				$this->assign('xml', $xml);
				break;
			case 3: $openid = $this->_get('openid');
				$where = array('token' => $this->token);
