...
KindEditor.plugin('table', function(K) {
	...
	self.plugin.table = {
		...
		colinsert : function(offset) {
			...
			// 取得第一行的index
			index += table.rows[0].cells.length - row.cells.length;		for (var i = 0, len = table.rows.length; i < len; i++) {
		...
		rowinsert : function(offset) {
			...
			var newRow = table.insertRow(rowIndex);		for (var i = 0, len = row.cells.length; i < len; i++) {
				// 调整cell个数
