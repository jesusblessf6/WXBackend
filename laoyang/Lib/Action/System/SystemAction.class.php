...
/*
  ...
  PRIMARY KEY (`id`)
        ...
        $remoteStr = @laoyang_getcontents($url);
    ...
    public function doUpdate()
    {
        ...
        if (!isset($_GET['ignore'])) {
            if (!is_writable(($_SERVER['DOCUMENT_ROOT'] . '/laoyang'))) {
            	$cannotWrite=1;
                $this->error('您的服务器laoyang文件夹不可写入，设置好再升级', U('System/main'));
            }
        ...
        if (intval($rt['success']) < 1) {
            ...
            file_put_contents($locationZipPath, @laoyang_getcontents($filename));
            $zip = new ZipArchive();
...
function laoyang_getcontents($url){
