...
class HuadianAction extends UserAction
{
    public function _initialize()
    {
      ...
      //      $this->error('您还开启该模块的使用权,请到功能模块中添加', U('Connme/index', array('token' => $this->token)));
        }
    }
    ...
    public function son_del()
    {
        ...
        $id = (int) $this->_get('id');
        $token = $this->_get('token');
        $where = array('id' => $id, 'token' => $token,'thetype' => 'huadian');
        $check = $t_son->where($where)->find();
        if ($check == null) {
            $this->error('操作失败');
        } else {
    ...
    public function housetype_add()
    {
        ...
        $son_data = M('Estate_son')->where(array('token' => session('token'),'thetype' => 'huadian'))->field('id as sid,title')->select();
        $this->assign('son_data', $son_data);
    ...
    public function album_add()
    {
        ...
        $check = $t_album->where(array('token' => session('token'), 'poid' => $poid,'thetype' => 'huadian'))->find();
        $this->assign('album', $check);
        if (IS_POST) {
            if ($check == NULL) {
                $check_ex = $t_album->where(array('token' => session('token'), 'poid' => $this->_post('poid'),'thetype' => 'huadian'))->find();
                if ($check_ex) {
