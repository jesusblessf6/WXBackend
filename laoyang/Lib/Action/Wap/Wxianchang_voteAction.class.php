<?php
class Wxianchang_voteAction extends WapAction{
    ...
    public function index(){
        ...
        if (!$member){
            header('location:' . U('Wxianchang_member/index', array('token' => $this -> token, 'wecha_id' => $this -> wecha_id, 'id' => $act_id, 'act_type' => $act_type, 'name' => 'vote')));
            exit();
        }
        $vid = M('Wechat_Wxianchang') -> where(array('token' => $this -> token, 'id' => $act_id)) -> getField('vote_id');
        if(empty($vid)){
            echo '参数错误';
            exit();
        }
        $vote_id = explode(',', $vid);
        $info = array();
        $where = array('token' => $this -> token, 'id' => array('in', $vote_id), 'type' => 'Wxianchang');
