...
class JiudianAction extends UserAction{
        ...
        if(IS_GET){                              
            ...
            if($back==true){
                ...
                $this->success('操作成功',U($this->type.'/infos',array('token'=>$this->token,'pid'=>$check['pid'])));
                 ...
                 $this->error('服务器繁忙,请稍后再试',U($this->type.'/infos',array('token'=>$this->token,'pid'=>$check['xid'])));
	...
	public function setType(){
        ...
        if(IS_GET){                              
			...
			if($this->yuyue_order->where($where)->setField($data)){
				if($type=='1')
				{
					$where1 = array('token'=> $this->token);
					$check1 = $this->wxuser->where($where1)->order('id desc')->select();
					$smsstatus = $check1[0]['smsstatus'];
					if($smsstatus=='1')
					{
						$smsuser = $check1[0]['smsuser'];
						$smspassword = $check1[0]['smspassword'];
						$check = $this->yuyue_order->where($where)->order('id desc')->select();
						$phone=$check[0]['phone'];
						$content="亲，您已经成功预定了".$check[0]['nums']."间".$check[0]['kind']."，请您记得在".$check[0]['or_date']." ".$check[0]['time']."之前过来办理入住手续。【".$check1[0]['subfix']."】";
						$smsapi = "api.smsbao.com"; //短信网关 
						$charset = "utf8"; //文件编码 
						$user = $smsuser; //短信平台帐号 
						$pass = md5($smspassword); //短信平台密码 
						include_once("snoopy.php"); 
						$snoopy = new snoopy();
						$sendurl = "http://{$smsapi}/sms?u={$user}&p={$pass}&m={$phone}&c=".urlencode($content);
						$snoopy->fetch($sendurl);
						$result = $snoopy->results;
						$result_text="失败，原因：代码出错！";
						switch ($result)
						{
						case 0:
						  $result_text="成功！";
						  break;
						case 30:
						  $result_text="失败，原因：密码错误 ！";
						  break;
						case 40:
						  $result_text="失败，原因：账号不存在！";
						  break;
						case 41:
						  $result_text="失败，原因：余额不足！";
						  break;
						case 42:
						  $result_text="失败，原因：帐号过期！";
						  break;
						case 43:
						  $result_text="失败，原因：IP地址限制！";
						  break;
						case 50:
						  $result_text="失败，原因：内容含有敏感词！";
						  break;
						case 51:
						  $result_text="失败，原因：手机号码不正确！";
						  break;
						default:
						  $result_text="失败，原因：未知错误！";
						}
						$this->success('修改成功！短信通知'.$result_text,U($this->type.'/infos',array('pid'=>$pid,'token'=>$this->token)));
					}
					else
					{
						$this->success('修改成功！短信功能未开启！',U($this->type.'/infos',array('pid'=>$pid,'token'=>$this->token)));
					}
				}
				else
				{
	...
	public function checksetinfo($str,$pid)
	{
		$result=M('setinfo')->where(array('token'=>$this->token,'type'=>$this->type,'pid'=>$pid,'name'=>$str))->select();
		
		if(!$result)
		{
			M('setinfo')->add(array('token'=>$this->token,'name'=>$str,'value'=>0,'kind'=>2,'type'=>$this->type,'pid'=>$pid));
		}
	}
	//订单设置
	 ...
	 public function setinfo(){ 
		...
		if(!empty($data)){
			...
			$this->checksetinfo("date1",$pid);//后续新增
			
			$this->checksetinfo("time1",$pid);//后续新增
			...
			$str=array("person" => 1 ,"phone" => 1 ,"date" => 1 ,"time" => 1,"date1" => 1 ,"time1" => 1,);
			...
			$setinfo->add(array('token'=>$this->token,'name'=>'date1','value'=>1,'kind'=>2,'type'=>$this->type,'pid'=>$pid));

			$setinfo->add(array('token'=>$this->token,'name'=>'time1','value'=>1,'kind'=>2,'type'=>$this->type,'pid'=>$pid));
		...
		if($check != 0 ){
			...
			$setinfo->where(array('token'=>$this->token,'name'=>'date1','type'=>$this->type,'pid'=>$pid))->save(array('value'=>$_POST['date1']));

			$setinfo->where(array('token'=>$this->token,'name'=>'time1','type'=>$this->type,'pid'=>$pid))->save(array('value'=>$_POST['time1']));
