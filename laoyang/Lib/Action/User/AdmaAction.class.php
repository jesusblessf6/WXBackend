...
class AdmaAction extends UserAction{
	public function index(){		
		...
		//dump($token_open);
		if(!strpos($token_open['queryname'],'adma')){$this->error('您还开启该模块的使用权,请到功能模块中添加',U('Connme/index',array('token'=>session('token'),'id'=>session('wxid'))));}
		$data=D('Adma');
		...
		if(IS_POST){
			...
			if($adma==false){
				//$adma=M('Adma')->where(array('token'=>'laoyang','uid'=>1))->find();
				$adma=array();
