				...
				switch ($pm['name']){
					case 'tel':
						if (!$pm['url']){
							...
							$pm['url']='http://site.tg.qq.com/forwardToPhonePage?siteId=1&phone={Laoyang:$thisCompany.tel}'.$pm['url'];
						}
						break;
					case 'memberinfo':
						if (!$pm['url']){
...
marker1.addEventListener("click", function(){this.openInfoWindow(infoWindow1);});
	...
	public function getLink($url){
		...
		if (strExists($url,'刮刮卡')){
			$link=Laoyang_URL.'/index.php?g=Wap&m=Guajiang&a=index&token='.$this->token.'&wecha_id='.$this->wechat_id;
			if ($itemid){
				$link.='&id='.$itemid;
			}
		}elseif (strExists($url,'大转盘')){
			$link=Laoyang_URL.'/index.php?g=Wap&m=Lottery&a=index&token='.$this->token.'&wecha_id='.$this->wechat_id;
			if ($itemid){
				$link.='&id='.$itemid;
			}
		}elseif (strExists($url,'优惠券')){
			$link=Laoyang_URL.'/index.php?g=Wap&m=Coupon&a=index&token='.$this->token.'&wecha_id='.$this->wechat_id;
			if ($itemid){
				$link.='&id='.$itemid;
			}
		}elseif (strExists($url,'商家订单')){
			if ($itemid){
				$link=$link=Laoyang_URL.'/index.php?g=Wap&m=Host&a=index&token='.$this->token.'&wecha_id='.$this->wechat_id.'&hid='.$itemid;
			}else {
				$link=Laoyang_URL.'/index.php?g=Wap&m=Host&a=Detail&token='.$this->token.'&wecha_id='.$this->wechat_id;
			}
		}elseif (strExists($url,'万能表单')){
			if ($itemid){
				$link=$link=Laoyang_URL.'/index.php?g=Wap&m=Selfform&a=index&token='.$this->token.'&wecha_id='.$this->wechat_id.'&id='.$itemid;
			}else {
				$link=Laoyang_URL.'/index.php?g=Wap&m=Host&a=Detail&token='.$this->token.'&wecha_id='.$this->wechat_id;
			}
		}elseif (strExists($url,'相册')){
			if ($itemid){
				$link=$link=Laoyang_URL.'/index.php?g=Wap&m=Photo&a=plist&token='.$this->token.'&wecha_id='.$this->wechat_id.'&id='.$itemid;
			}else {
				$link=Laoyang_URL.'/index.php?g=Wap&m=Photo&a=index&token='.$this->token.'&wecha_id='.$this->wechat_id;
			}
		}elseif (strExists($url,'会员卡')){
			$link=Laoyang_URL.'/index.php?g=Wap&m=Card&a=vip&token='.$this->token.'&wecha_id='.$this->wechat_id;
		}elseif (strExists($url,'商城')){
			$link=Laoyang_URL.'/index.php?g=Wap&m=Product&a=index&token='.$this->token.'&wecha_id='.$this->wechat_id;
		}elseif (strExists($url,'订餐')){
			$link=Laoyang_URL.'/index.php?g=Wap&m=Product&a=dining&dining=1&token='.$this->token.'&wecha_id='.$this->wechat_id;
		}elseif (strExists($url,'团购')){
			$link=Laoyang_URL.'/index.php?g=Wap&m=Groupon&a=grouponIndex&token='.$this->token.'&wecha_id='.$this->wechat_id;
		}elseif (strExists($url,'网站分类')){
			$link=Laoyang_URL.'/index.php?g=Wap&m=Index&a=lists&token='.$this->token.'&wecha_id='.$this->wechat_id;
			if ($itemid){
				$link=Laoyang_URL.'/index.php?g=Wap&m=Index&a=lists&token='.$this->token.'&wecha_id='.$this->wechat_id.'&classid='.$itemid;
			}
		}elseif (strExists($url,'图文回复')){
			if ($itemid){
				$link=Laoyang_URL.'/index.php?g=Wap&m=Index&a=index&token='.$this->token.'&wecha_id='.$this->wechat_id.'&id='.$itemid;
			}
		}elseif (strExists($url,'LBS信息')){
			$link=Laoyang_URL.'/index.php?g=Wap&m=Company&a=map&token='.$this->token.'&wecha_id='.$this->wechat_id;
			if ($itemid){
				$link=Laoyang_URL.'/index.php?g=Wap&m=Company&a=map&token='.$this->token.'&wecha_id='.$this->wechat_id.'&companyid='.$itemid;
			}
		}elseif (strExists($url,'DIY宣传页')){
			$link=Laoyang_URL.'/index.php/show/'.$this->token;
		}elseif (strExists($url,'婚庆喜帖')){
			if ($itemid){
				$link=Laoyang_URL.'/index.php?g=Wap&m=Wedding&a=index&token='.$this->token.'&wecha_id='.$this->wechat_id.'&id='.$itemid;
			}
		}elseif (strExists($url,'投票')){
			if ($itemid){
				$link=Laoyang_URL.'/index.php?g=Wap&m=Vote&a=index&token='.$this->token.'&wecha_id='.$this->wechat_id.'&id='.$itemid;
			}
		}elseif (strExists($url,'全景')){
			$link=Laoyang_URL.'/index.php?g=Wap&m=Panorama&a=index&token='.$this->token.'&wecha_id='.$this->wechat_id;
			if ($itemid){
				$link=Laoyang_URL.'/index.php?g=Wap&m=Panorama&a=item&token='.$this->token.'&wecha_id='.$this->wechat_id.'&id='.$itemid;
			}
		}else {
			$link=str_replace(array('{wechat_id}','{siteUrl}'),array($this->wechat_id,Laoyang_URL),$url);
		}
		return $link;
	}
