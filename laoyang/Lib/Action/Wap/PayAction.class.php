...
class PayAction extends WapAction{
	...
	public function _initialize() {
		...
		if (!$this->orderid){
			$this->orderid=$_GET['single_orderid'];//单个订单
		}	
		...
		//免签支付宝配置
		$pay_config =M('payment')->where(array('token'=>$this->token,'pay_code'=>'bwalipay'))->find();
		$pay_config = unserialize($pay_config['pay_config']);
	...
	public function wapalipay(){
		...
		$call_back_url= $this->company_site_url.'/index.php/Wap/Pay/wapalipay_call_back_url?from='.$_GET['from'].'|'.$_GET['token'].'|'.$_GET['id'];
	...
	public function bwalipay(){
		...
		$return_url= $this->company_site_url.'/index.php?g=Wap&m=Pay&a=bwalipay_return_url&token='.$_GET['token'].'&wecha_id='.$_GET['wecha_id'].'&from='.$_GET['from'].'&id='.$_GET['id'];
		//付款金额
	...
	public function wxpay(){
		$pay_url= $this->company_site_url."/wxpay/?g=Wap&m=Wxpay&a=pay&token=".$this->token."&wecha_id=".$this->wecha_id."&orderid=".$this->orderid."&from=".$_GET['from']."&id=".$_GET['id'];
		header("location:".$pay_url);
	}
	public function baofu(){
		$pay_url= $this->company_site_url."/?g=Wap&m=Baofu&a=pay&token=".$this->token."&wecha_id=".$this->wecha_id."&orderid=".$this->orderid."&from=".$_GET['from']."&id=".$_GET['id'];
		header("location:".$pay_url);
	}
	public function offlinepay(){
		$pay_url= $this->company_site_url."/?g=Wap&m=Offlinepay&a=pay&token=".$this->token."&wecha_id=".$this->wecha_id."&orderid=".$this->orderid."&from=".$_GET['from']."&id=".$_GET['id'];
		header("location:".$pay_url);
	}	
	...
	{	
		...
		if($verify_result) {
			...
			if($result == "success") {
				//after
				$payHandel=new payHandle($arr[1],$arr[0],'alipay');
				$orderInfo=$payHandel->afterPay($out_trade_no,$trade_no);
				$from=$payHandel->getFrom();
				$this->redirect('/index.php?g=Wap&m='.$from.'&a=payReturn&token='.$orderInfo['token'].'&wecha_id='.$orderInfo['wecha_id'].'&orderid='.$out_trade_no.'&id='.$arr[2]);
			}else {
			  exit('付款失败');
			}
		}else {
	...
	public function bwalipay_return_url(){	
		...
		//if($verify_result) {
			...
			if( $this->_get('trade_status') == 'TRADE_FINISHED' ||  $this->_get('trade_status') == 'TRADE_SUCCESS') {
				...
				$orderInfo=$payHandel->afterPay($out_trade_no,$trade_no);
				$from=$payHandel->getFrom();
				$this->redirect('/index.php?g=Wap&m='.$from.'&a=payReturn&token='.$orderInfo['token'].'&wecha_id='.$orderInfo['wecha_id'].'&orderid='.$out_trade_no.'&id='.$this->_get('id'));
			}else {
			  exit('付款失败');
			}
		//}else {
	...
	public function tenpay(){
		...
		$return_url= $this->company_site_url.'/index.php?g=Wap&m=Pay&a=tenpayshare_return_url&token='.$_GET['token'].'&wecha_id='.$_GET['wecha_id'].'&from='.$_GET['from'].'&id='.$_GET['id'];
	...
	public function waptenpay(){	
		...
		$return_url = $this->company_site_url.'/index.php?g=Wap&m=Pay&a=return_url&token='.$this->token.'&wecha_id='.$this->wecha_id.'&from='.$_GET['from'].'&id='.$_GET['id'];
	...
	public function tenpayshare_return_url (){
		...
		$key=trim($this->tenpay_config['partnerKey']);
		...
		if("0" == $trade_state) {
			...
			$orderInfo=$payHandel->afterPay($out_trade_no,$trade_no);
			$from=$payHandel->getFrom();
			$this->redirect('/index.php?g=Wap&m='.$from.'&a=payReturn&token='.$orderInfo['token'].'&wecha_id='.$orderInfo['wecha_id'].'&orderid='.$out_trade_no.'&id='.$_GET['id']);
		}else{
	...
	public function return_url (){
		...
		if($resHandler->isTenpaySign()) {
			...
			if( "0" == $pay_result  ) {
				...
				$this->redirect('?g=Wap&m='.$from.'&a=payReturn&token='.$_GET['token'].'&wecha_id='.$thisOrder['wecha_id'].'&orderid='.$out_trade_no.'&id='.$_GET['id']);
			} else {
				//当做不成功处理
				$string =  "<br/>" . "支付失败" . "<br/>";
				echo $string;
