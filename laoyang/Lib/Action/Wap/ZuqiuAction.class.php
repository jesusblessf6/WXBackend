...
class ZuqiuAction extends LotteryBaseAction{
	public function index()
	{
		...
		M('Lottery')->where(array('id'=>$id))->setInc('click');
		if($record == Null){
			$redata->add($where);
			$record =$redata->where($where)->find();
		}
		...
		$queue = array();
		$qiu = 'q_'.$_GET['wecha_id'].'_'.$_GET['id'];
		$wecha_id = 'z_'.$_GET['wecha_id'].'_'.$_GET['id'];
		$winprizeAll = 'All_'.$_GET['wecha_id'].'_'.$_GET['id'];
		$winprizes = 's_'.$_GET['wecha_id'].'_'.$_GET['id'];
		
		$winprize = $_SESSION[$winprizes];
		if(empty($winprize))
		{	
			$_SESSION[$winprizes] = $data['winprize'];
			$winprize = $_SESSION[$winprizes];
		}
	...
	if($shu==5)
	{
		$_SESSION[$winprizeAll] = 2;
	}
	else
	{
		$zhi = $_SESSION[$winprizeAll];
		$page = $_SESSION[$qiu];
		...
		if(isset($zhi))
		{
			if($zhi!=2)
			{
				...
				{
					$_SESSION[$winprizeAll] = 2;
					$username = $_SESSION[$winprizes];
					if($username!='谢谢参与' and $username!='抽奖次数已用完')
					{
						$this->assign('winprize',3);
						$this->assign('usenums',1);
						$this->assign('winprizes',$username);
				...
				{
					$queue = $_SESSION[$wecha_id];
					$page = $page+1;
					$_SESSION[$qiu]=$_SESSION[$qiu]+1;
					$_SESSION[$winprizeAll] = $queue[$page];
				}
			}
			else
			{
		...
		{
			if($shu==1)
				$queue=array('1','1','1','1','1','1');
			if($shu==2)
				$queue=array('0','0','1','1','1','1');
			if($shu==3)
				$queue=array('0','0','0','1','1','1');
			if($shu==4)
				$queue=array('0','0','0','0','0','0');
			shuffle($queue);
			//dump($queue);
			$_SESSION[$wecha_id] = $queue;
			$_SESSION[$qiu] = 0;
			$_SESSION[$winprizeAll] = $queue[0];
		}
	}
		$this->assign('winprize',$_SESSION[$winprizeAll]);
		$this->assign('prizeStr',$prizeStr);
		$this->display();
