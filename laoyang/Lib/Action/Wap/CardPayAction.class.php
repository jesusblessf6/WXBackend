...
//调用地址 Wap/CardPay/pay ;参数：from price single_orderid orderName token wecha_id redirect（Moudle/Action|param1:value1,param2:value2）
		...
		//已输入密码
			if($_POST['pass'] != false){
				if(md5($_POST['pass']) == $paypass){
					$this->gotopay($single_orderid,$price,$from,$orderName,$redirect,$id);
				}else{
					$this->error('密码错误');
				}	
			}
		...
		private function gotopay($orderid,$price,$from,$orderName,$redirect=NULL,$id){
						...
						if($flag1 && $flag2){
							...
							$payrecord->where(array('orderid'=>$orderid,'token'=>$this->token))->setField('paid',1);
								...
								if ($redirect){
									...
									$this->redirect(U("$from/payReturn",array('orderid'=>$orderid,'token'=>$this->token,'wecha_id'=>$this->wecha_id,'id'=>$id,'paytype'=>'cardPay')));
								}
