...
include_once("wx.class.php"); //微信具体函数
...
class WxPayHelper
{
	...
	* 生成调取收货地址请求json
	*/
	function create_addr_package($url){
		...
		try {
			...
			$bizObj["accesstoken"]=$access_token;
			$bizObj["appId"]=$this->appid;
			$bizObj["noncestr"]=$noncestr;
			$bizObj["timestamp"]=$timestamp;
			$bizObj["url"]=$url;
			...
			$nativeObj["addrSign"] = $this->get_address_sign($bizObj);
	...
	//生成地址共享签名
	function get_address_sign($bizObj){
