...
class StoreAction extends BaseAction{
	...
	public function _initialize() {
		...
		$cats = $this->product_cat_model->where(array('token' => $this->token,'dining' => 0))->order('id asc')->select();
	...
	public function product() {
        ...
        if($productCatData = M('Product_cat')->where(array('id' => $product['catid'], 'token' => $this->token,'dining' => 0))->find()) {
	...
	public function ordersave() {
		...
		if ($carts){
			...
			//TODO 发货的短信提醒
						if($this->thisCompany['print_status']==1)
				{
					if($this->thisCompany['printtype']=='feiyin')
					{
						$printercounts=$this->email_body();
						define('MEMBER_CODE', $this->thisCompany['memberCode']);
						define('FEYIN_KEY', $this->thisCompany['feiyin_key']);
						define('DEVICE_NO', $this->thisCompany['deviceNo']);
						define('Page',$this->thisCompany['printpage']);
					...
					if($this->thisCompany['printtype']=='laoyang')
					{
						$url = 'http://115.28.141.183:60002';//POST指向的API链接 
						$printercounts=$this->print2_body();
						define('DEVICE_NO', $this->thisCompany['bwdeviceNo']);
						define('Page',$this->thisCompany['printpage']);
						$str=$printercounts;
						$msgInfo=array(
				...
				if($row['payment']==1){
					...
					$phone=$this->thisCompany['phone'];
					
					$user=$info['smsuser'];//短信平台帐号
					$pass=md5($info['smspassword']);//短信平台密码
					$smsstatus=$this->thisCompany['phone_status'];//短信平台状态
					$content = $this->sms_body();
					$content = $content.'【'.$info['subfix'].'】'; //加后缀
					...
					//给管理员发邮件通知
					$to_email=$this->thisCompany['email'];
					$emailuser=$info['emailuser'];
					$emailpassword=$info['emailpassword'];
					$emailstatus=$this->thisCompany['email_status'];
	...
	public function send_email($Subject,$body,$emailuser,$emailpassword,$to_email){
		...
		if ($normal_rt){
			...
			if ($pay_code&&$pay_code!=3){
				$this->redirect(U('Pay/'.$pay_code,array('token'=>$this->token,'wecha_id'=>$this->wecha_id,'orderid'=>$orderid)));
			}else {
				$this->redirect(U('Store/my',array('token'=>$this->token,'wecha_id'=>$this->wecha_id,'success'=>1)));
			}
	...
	public function payReturn() {
	   ...
	   if ($order = M('Product_cart')->where(array('orderid' => $orderid, 'token' => $this->token))->find()) {
	      ...
	      exit('订单不存在');
					...
					// 结束
					$this->success('下单成功！',U('Store/my',array('token'=>$this->token,'wecha_id'=>$this->wecha_id,'success'=>1)));
				}
