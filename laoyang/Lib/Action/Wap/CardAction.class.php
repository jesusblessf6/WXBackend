...
class CardAction extends WapAction{
	...
	public function index(){
		...
		if ($cardsCount&&isset($_GET['mycard'])){
			$this->redirect(U('Card/card',array('token'=>$this->token,'wecha_id'=>$this->wecha_id,'cardid'=>$cards[0]['cardid'])));
		}
    ...
    public function addSign(){
		...
		$whereinfo =  array('token'=>$this->token,'wecha_id'=>$this->wecha_id);
    	...
    	if($sign == null){
			...
			M('Userinfo')->where($whereinfo)->save($da);  
    		 echo'{"success":1,"msg":"签到成功，获得'.$set_exchange['everyday'].'积分！"}';exit();
    	}
	   ...
	   if($userinfo['continuous'] == 6){
			...
			$da['sign_score']  = $userinfo['sign_score'] + $data['expense'];
			$da['total_score'] = $userinfo['total_score'] + $data['expense'];
			$da['continuous']  = 1; //清空计数器
			M('Userinfo')->where($whereinfo)->save($da);  
			echo'{"success":1,"msg":"签到成功，获得'.$set_exchange['everyday'].'积分，奖励'.$set_exchange['continuation'].'积分"}';
	   }else{
			$sign = $cardsign_db->where($where)->order('id desc')->find();
			//是否是连续签到，如果不是，清空计数器
			$itoday = date('Y-m-d',intval($sign['sign_time']));	
			$ytoday = date('Y-m-d',strtotime('-1 day'));
			if ($itoday!=$ytoday) {  // 判断时间是否大于24小时
				   //exit('清零计数器，继续签到');
				   ...
				   M('Userinfo')->where($whereinfo)->save($da);
					...
					$da['continuous']  = 1;
					M('Userinfo')->where($whereinfo)->save($da); 
					 echo'{"success":1,"msg":"签到成功，获得'.$set_exchange['everyday'].'积分！"}';
			}else{
				//是连续签到
			     //exit('是连续签到 ');
				$data['sign_time']  = time();
	...
	public function vip(){
		...
		$card=M('member_card_create')->where(array('token'=>$token,'wecha_id'=>$wecha_id))->find();
		if($card==false){
			header('Location:'.rtrim(C('site_url'),'/').U('Wap/Card/index',array('token'=>$token,'wecha_id'=>$wecha_id)));
		}
		//
	   $agent = $_SERVER['HTTP_USER_AGENT']; 
		if(!strpos($agent,"MicroMessenger")) {
	...
	public function offlinepay(){
		...
		$token = $this->token;
		$thisCompany=M('Company')->where("token = '$token'")->find();
		$this->assign('thisCompany',$thisCompany);
		$this->assign('info',$info);
	...
	public function offlinepayAction(){
		...
		if (IS_POST){
			if($_POST['orderid'])
			{
				...
				$this->success('提交成功，正在跳转支付页面..',U('Pay/'.$_POST['payment'],array('from'=>'Card','orderName'=>$_POST['ordername'],'token'=>$_POST['token'],'wecha_id'=>$_POST['wecha_id'],'orderid'=>$_POST['orderid'],'price'=>$_POST['price'])));
			}
			else
			{
				if($_POST['price'] <= 0){
			...
			if($order['paid'] == 1){
				...
				if($order['type'] == 1){
					if($order['intogold'] == 0)
					{
						...
						$record -> where(array('orderid' => $orderid)) -> setField('intogold', 1);
						//订单通知
						$staff_db=M('Company_staff');
						$thisStaff=$staff_db->where(array('username'=>$order['operator'],'token'=>$token))->getField('tel');
						if($thisStaff)
						{
							Sms::sendSms($token,$order['ordername']."，已充值".$order['price']."，操作员：".$order['operator'],$thisStaff);
						}
					}
				}else{
					...
					if($this->_get('type') == 'coupon'){
						...
						M('Member_card_use_record')->where(array('token'=>$token,'id'=>$lastid))->setField(array('usecount'=>(int)$this->_get('usecount'),'cat'=>3));
					}elseif($this->_get('type') == 'privelege'){
						M('Member_card_vip')->where(array('token'=>$token,'id'=>(int)$this->_get('itemid')))->setInc('usetime');
						M('Member_card_use_record')->where(array('token'=>$token,'id'=>$lastid))->setField('cat',1);
					}
				}
