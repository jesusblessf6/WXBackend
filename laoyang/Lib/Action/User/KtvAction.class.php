...
class KtvAction extends UserAction
{
    ...
    public function album_add()
    {
        ...
        $check = $t_album->where(array('token' => session('token'), 'poid' => $poid,'thetype' => 'ktv'))->find();
        $this->assign('album', $check);
        if (IS_POST) {
            if ($check == NULL) {
                $check_ex = $t_album->where(array('token' => session('token'), 'poid' => $this->_post('poid'),'thetype' => 'ktv'))->find();
                if ($check_ex) {
