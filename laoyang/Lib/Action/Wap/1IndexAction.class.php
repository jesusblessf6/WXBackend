    ...
    public function index()
    {
        ...
        foreach ($info as $k => $v) {
            ...
            if ($v['sub'] != NULL) {
                foreach ($v['sub'] as $ke => $va) {
                    if ($v['sub'][$ke]['url'] == '') {
                        $info[$k]['sub'][$ke]['url'] = U('Index/lists', array(
                            'classid' => $v['sub'][$ke]['id'],
                            'token' => $where['token'],
                            'wecha_id' => $this->wecha_id
                        ));
        ...
        if ($tpldata['tpltypename'] == 'ktv_list' || $tpldata['tpltypename'] == 'yl_list') {
            ...
            foreach ($info as $key => $val) {
                ...
                if ($info[$key]['url'] == '') {
                    $info[$key]['url'] = U('Index/lists', array(
                        'classid' => $val['id'],
                        'token' => $where['token'],
                        'wecha_id' => $this->wecha_id
                    ));
        ...
        if (!empty($sub) AND empty($imgdata)) {
            ...
            if ($tpldata['tpltypename'] == 'ktv_list' || $tpldata['tpltypename'] == 'yl_list') {
                ...
                foreach ($sub as $key => $val) {
                    ...
                    $sub[$key]['pic']   = $val['img'];
                    if ($sub[$key]['url'] == '') {
                        $sub[$key]['url'] = U('Index/lists', array(
                            'classid' => $val['id'],
                            'token' => $where['token'],
                            'wecha_id' => $this->wecha_id
                        ));
            ...
            foreach ($sub as $ke => $va) {
                ...
                if ($sub[$ke]['url'] == '') {
                    $sub[$ke]['url'] = U('Index/lists', array(
                        'classid' => $va['id'],
                        'token' => $where['token'],
                        'wecha_id' => $this->wecha_id
                    ));
            ...
            foreach ($res as $key => $val) {
                ...
                $res[$key]['img']  = $val['pic'];
                if ($res[$key]['url'] == '') {
                    $res[$key]['url'] = U('Index/content', array(
                        'id' => $val['id'],
                        'classid' => $val['classid'],
                        'token' => $where['token'],
                        'wecha_id' => $this->wecha_id
            ...
            foreach ($plugmenus as $pm) {
                switch ($pm['name']) {
                    ...
                    case 'membercard':
                        ...
                        if (!$pm['url']) {
                            if ($card == false) {
                                $pm['url'] = rtrim($this->siteUrl, '/') . U('Wap/Card/index', array(
                                    'token' => $this->token,
                                    'wecha_id' => $this->wecha_id
                                ));
                            } else {
