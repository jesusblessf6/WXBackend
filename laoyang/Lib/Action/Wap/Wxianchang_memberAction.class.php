<?php
class Wxianchang_memberAction extends WapAction{
    ...
    public function index(){
        ...
        if(!empty($mInfo) && $this -> _get('name') == 'vote'){
            header("Location:" . $this -> siteUrl . U('Wxianchang_vote/index', array('token' => $this -> token, 'wecha_id' => $this -> wecha_id, 'id' => $act_id, 'act_type' => $act_type)));
        }
    ...
    public function result($token, $act_name, $act_id, $act_type, $is_add){
        ...
        if($act_name == 'shake'){
            ...
            echo json_encode(array('err' => 0, 'name' => $act_name, 'href' => $this -> siteUrl . U('Wxianchang_vote/index', array('token' => $this -> token, 'wecha_id' => $this -> wecha_id, 'id' => $act_id, 'act_type' => $act_type))));
            exit();
        }else{
